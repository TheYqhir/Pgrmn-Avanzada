# ANALYSIS.md — Comparación de rendimiento
...
1. **Límite `√m`:** si `m` tiene un divisor, uno de ellos es necesariamente ≤ `√m`. Esto reduce las iteraciones de O(m) a O(√m) por candidato.
2. **`break` inmediato:** al encontrar el primer divisor se sale del bucle, evitando seguir probando divisores innecesarios.
...
| Archivo           | Complejidad base | Complejidad opt | Speedup |
|-------------------|------------------|-----------------|---------|
| `python_no_opt.py`| O(n²)            | O(n)            | ~5.7x   |
| `c_no_opt.c`      | O(n × m)         | O(n × √m)       | ~87x    |
